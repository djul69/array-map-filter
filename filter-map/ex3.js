...
function getStudentsPerCurriculum(campuses, curriculumName) {
  const result=campuses.filter(myCursus=>myCursus.curriculums[0].name===curriculumName);
 return result.map(function(myCursusName){
   var obj ={};
   var key=myCursusName.city;
   obj[key]=myCursusName.curriculums[0].numStudents;
   return obj;
 });}
